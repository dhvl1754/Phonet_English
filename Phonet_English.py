...
class Phonet_English:
    ...
    def load_model(self):
        ...
        model_file="Trained Model/model.h5"     #----------------Load the model for trained for probablities
    ...
    def load_model_phon(self):
        input_size=(self.len_seq, self.nfeat)
        Model_phonemes="Trained Model/phonemes_weights.hdf5"   #---------------Load the model for Phoneme recognition
        Model_phon=self.modelp(input_size)
        Model_phon.load_weights(Model_phonemes)
        return Model_phon

    def load_scaler(self):
        file_mu="Trained Model/mu.npy"                                 #----------------Load the files
        file_std="Trained Model/std.npy"                               #----------------Load the files
        MU=np.load(file_mu)
